/* $Id: soundmixer.c,v 1.20 2002/04/10 23:10:10 sandervl Exp $ */
/*
...
OSSRET OSS32_MixSetVolume(OSSSTREAMID streamid, ULONG line, ULONG volume)
{
    ...
    //TODO: This volume conversion method doesn't work well
    lVol = GET_VOLUME_L(volume);
    lVol = (lVol * pElemInfo->value.integer.max) / OSS32_MAX_VOLUME;
    pElem->value.integer.value[0] = lVol;

    if(pElemInfo->count > 1) { //stereo
        rVol = GET_VOLUME_R(volume);
        rVol = (rVol * pElemInfo->value.integer.max) / OSS32_MAX_VOLUME;
        pElem->value.integer.value[1] = rVol;
    }

    dprintf(("OSS32_MixSetVolume of %s to (%d,%d)(%d,%d)", pHandle->pids[idx].name, GET_VOLUME_L(volume), GET_VOLUME_R(volume), lVol, rVol));
    ret = pHandle->file.f_op->ioctl(&pHandle->inode, &pHandle->file, SNDRV_CTL_IOCTL_ELEM_WRITE, (ULONG)pElem);
...
levelcontinue:
        ...
        //TODO: This volume conversion method doesn't work well
        lVol = GET_VOLUME_L(ulValue);
        lVol = (lVol * pElemInfo->value.integer.max) / OSS32_MAX_VOLUME;
        pElem->value.integer.value[0] = lVol;

        if(pElemInfo->count > 1) { //stereo
            rVol = GET_VOLUME_R(ulValue);
            rVol = (rVol * pElemInfo->value.integer.max) / OSS32_MAX_VOLUME;
            pElem->value.integer.value[1] = rVol;
        }
        break;
...
OSSRET OSS32_QueryNames(ULONG deviceid, char *pszDeviceName, ULONG cbDeviceName,
                        char *pszMixerName, ULONG cbMixerName)
{
    mixerhandle *pHandle = NULL;
    int          ret, i, j;
