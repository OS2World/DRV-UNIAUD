/* $Id: idc.c,v 1.17 2002/03/18 19:19:14 sandervl Exp $ */
/*
...
OSSRET OSS32IDC(ULONG cmd, PIDC32_PACKET pPacket)
{
  ...
  case IDC32_WAVE_OPEN:
  {
       ...
       rc = OSS32_WaveOpen(pPacket->open.devicenr, pPacket->open.streamtype, &streamId);
  ...
  {
       ULONG transferred = 0;
       rc = OSS32_WaveAddBuffer(pPacket->streamid, pPacket->buffer.buffer, pPacket->buffer.size, &transferred);
  ...
  case IDC32_MIDI_READ:
       return OSS32_MidiRead(pPacket->streamid, (char *)__Stack16ToFlat(pPacket->midiread.buffer), pPacket->midiread.bufsize);
