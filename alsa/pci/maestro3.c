...
#define CARD_NAME "ESS Maestro3/Allegro"
...
#ifdef TARGET_OS2
static int snd_enable[SNDRV_CARDS] = {REPEAT_SNDRV(1)}; /* all enabled */
static int snd_external_amp[SNDRV_CARDS] = {REPEAT_SNDRV(1)};
#else
static int snd_enable[SNDRV_CARDS] = {[0 ... (SNDRV_CARDS - 1)] = 1}; /* all enabled */
...
#endif
{
	...
	for (i = 2; i < 0x26 ; i += 2)
		snd_ac97_write(chip->ac97, i, chip->ac97->regs[i]);
	...
	if (chip->substreams) {
		...
		for (i = 0; i < chip->num_substreams; i++) {
			m3_dma_t *s = &chip->substreams[i];
			if (s->substream && s->running) {
				snd_m3_pcm_start(chip, s, s->substream);
			}
		}
		spin_unlock_irqrestore(&chip->reg_lock, flags);
	}
...
static void __exit snd_m3_remove(struct pci_dev *pci)
{
