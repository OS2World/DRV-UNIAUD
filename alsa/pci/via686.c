...
static int snd_joystick[SNDRV_CARDS] = {REPEAT_SNDRV(-1)};
...
static inline unsigned int snd_via686a_cur_ptr(via686a_t *chip, viadev_t *viadev)
{
	...
	if (!(inb(VIAREG(chip, OFFSET_STATUS) + viadev->reg_offset) & VIA_REG_STAT_ACTIVE))
		return 0;
	// tmp =
	val = (((ptr - viadev->table_addr) / 8) - 1) % viadev->frags;
	val *= viadev->fragsize;
...
static snd_pcm_ops_t snd_via686a_playback_ops = {
	snd_via686a_playback_open,
	snd_via686a_playback_close,
	snd_pcm_lib_ioctl,
	snd_via686a_hw_params,
	snd_via686a_hw_free,
	snd_via686a_playback_prepare,
	snd_via686a_playback_trigger,
	snd_via686a_playback_pointer,0,0
};

static snd_pcm_ops_t snd_via686a_capture_ops = {
	snd_via686a_capture_open,
	snd_via686a_capture_close,
	snd_pcm_lib_ioctl,
	snd_via686a_hw_params,
	snd_via686a_hw_free,
	snd_via686a_capture_prepare,
	snd_via686a_capture_trigger,
	snd_via686a_capture_pointer,0,0
};
#else
static snd_pcm_ops_t snd_via686a_playback_ops = {
...
static int __init snd_via686a_probe(struct pci_dev *pci,
				    const struct pci_device_id *id)
{
...
#ifdef TARGET_OS2
	if ((err = snd_via686a_mixer(chip)) < 0) {
#else
...
#ifdef TARGET_OS2
	if ((err = snd_via686a_pcm(chip, pcm_dev++, NULL)) < 0) {
#else
	...
	if (snd_joystick[dev] > 0) {
		if (rev_h && (legacy & 0x40) && (legacy & 8) == 0)
			outw(0x200, pci_resource_start(pci, 2) + 2);
		legacy |= 8;
	}
	if (rev_h && (legacy & 0x40) && snd_mpu_port[dev] >= 0x200) { /* rev H+ */
		legacy |= 0x02;	/* enable MPU */
		outw(snd_mpu_port[dev], pci_resource_start(pci, 2) + 0);
	} else {
		if (rev_h & (legacy & 0x40) && (legacy & 0x02)) {
			snd_mpu_port[dev] = inw(pci_resource_start(pci, 2) + 0);
		} else {
			switch ((legacy >> 2) & 3) {
...
static void __exit snd_via686a_remove(struct pci_dev *pci)
{
