...
#ifdef TARGET_OS2
static int snd_ac97_clock[SNDRV_CARDS] = {REPEAT_SNDRV(48000)};
#else
...
static int snd_via8233_codec_ready(via8233_t *chip, int secondary)
{
	...
	time = 1000;
	do {
		/* delay first, so the chip gets a chance to set busy */
		udelay(1);
		if (!(snd_via8233_codec_xread(chip) & VIA_REG_AC97_BUSY))
			return 0;
	} while (time--);
...
static unsigned short snd_via8233_codec_read(ac97_t *ac97, unsigned short reg)
{
	 ...
	 * The codec read command on my system sometimes fails, so
	 * we try 10 times (ac97 registers have no read side effects).
	 * An access normally takes about 20 usec.
	 */
	val = 0 << VIA_REG_AC97_CODEC_ID_SHIFT;
	val |= VIA_REG_AC97_ANY_VALID;		/* reset all valid bits */
	val |= VIA_REG_AC97_READ;
	val |= reg << VIA_REG_AC97_CMD_SHIFT;
	spin_lock(&chip->reg_lock);
	for (j=10; j--; ) {
		snd_via8233_codec_ready(chip, ac97->num);
		snd_via8233_codec_xwrite(chip, val);
		snd_via8233_codec_xread(chip);			/* flush pipeline  */
		udelay(10);					/* allow some time to set busy bit */
		for (i=100; i--;) {
			...
			if (!(val & VIA_REG_AC97_BUSY))	/* not busy and no valid data ? */
				break;				/* retry access */
			udelay(1);
		}
	}
...
static void snd_via8233_setup_fragments(via8233_t *chip, viadev_t *viadev,
...
{
	spin_lock(&chip->update_lock);
	viadev->position = inb(VIAREG(chip, OFFSET_CURR_INDEX) + viadev->reg_offset) * viadev->fragsize;
	viadev->position %= viadev->size;
	spin_unlock(&chip->update_lock);
	snd_pcm_period_elapsed(viadev->substream);
	outb(VIA_REG_STAT_FLAG | VIA_REG_STAT_EOL, VIAREG(chip, OFFSET_STATUS) + viadev->reg_offset);
	//printk("snd_via8233_update: position = %x  frag = %d\n", viadev->position, viadev->position/viadev->fragsize);
}
...
static int snd_via8233_playback_prepare(snd_pcm_substream_t * substream)
{
	...
	chip->playback.position = 0;
	snd_ac97_set_rate(chip->ac97, AC97_PCM_FRONT_DAC_RATE, runtime->rate);
	snd_via8233_setup_fragments(chip, &chip->playback, runtime);
...
static int snd_via8233_capture_prepare(snd_pcm_substream_t * substream)
{
	...
	chip->capture.position = 0;
	snd_ac97_set_rate(chip->ac97, AC97_PCM_LR_ADC_RATE, runtime->rate);
	snd_via8233_setup_fragments(chip, &chip->capture, runtime);
...
static snd_pcm_uframes_t snd_via8233_playback_pointer(snd_pcm_substream_t * substream)
{
	...
	ptr = chip->playback.position +
		(chip->playback.fragsize -
		(inl(VIAREG(chip, PLAYBACK_CURR_COUNT)) & 0xffffff));
...
static snd_pcm_uframes_t snd_via8233_capture_pointer(snd_pcm_substream_t * substream)
{
	...
	if (!(inb(VIAREG(chip, CAPTURE_STATUS)) & VIA_REG_STAT_ACTIVE))
		return 0;
	ptr = chip->capture.position +
		(chip->capture.fragsize -
		(inl(VIAREG(chip, CAPTURE_CURR_COUNT)) & 0xffffff));
	return bytes_to_frames(substream->runtime, ptr);
}
...
static int snd_via8233_chip_init(via8233_t *chip)
{
	...
	for (i=HZ; i--; ) {
		...
		if (stat & 1)
			break;
		if (!i) {
			snd_printk("chip_init: failed to access pimary codec.\n");
			return ~0;
		}
...
static int __init snd_via8233_probe(struct pci_dev *pci,
...
static void __exit snd_via8233_remove(struct pci_dev *pci)
{
